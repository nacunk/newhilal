...
def get_yallop_status(q_value):
    ...
    # Implementasi manual dengan 6 kategori sesuai standar Yallop
    if q_value >= 0.216:
        return "Mudah terlihat dengan mata telanjang"
    elif q_value >= -0.014:
        return "Terlihat dalam kondisi atmosfer sempurna"
    elif q_value >= -0.160:
        return "Mungkin memerlukan alat bantu optik"
    elif q_value >= -0.232:
        return "Hanya terlihat dengan alat bantu optik"
    elif q_value >= -0.293:
        return "Tidak terlihat dengan teleskop konvensional"
    else:
        return "Tidak terlihat (di bawah batas Danjon)"

# Fungsi untuk simulasi data historis (REVISI AKHIR - DIPERBAIKI)
def get_historical_data(start='2024-01-01', end='2024-12-31',
    ...
    try:
        ...
        # PERBAIKAN: Cari semua ijtimak dalam rentang dengan metode yang lebih akurat
        t0 = ts.utc(start_date.year, start_date.month, start_date.day)
        t1 = ts.utc(end_date.year, end_date.month, end_date.day, 23, 59, 59)
        
        # PERBAIKAN: Gunakan almanac.moon_phases dengan filter yang tepat
        t, y = almanac.find_discrete(t0, t1, almanac.moon_phases(eph))
        
        # PERBAIKAN: Filter hanya new moon (phase = 0) dan pastikan dalam rentang
        new_moon_times = []
        for ti, phase in zip(t, y):
            if phase == 0:  # New moon
                ijtimak_date = ti.utc_datetime().date()
                # Pastikan tanggal ijtimak dalam rentang yang diminta
                if start_date <= ijtimak_date <= end_date:
                    new_moon_times.append(ti)
        
        st.info(f"Ditemukan {len(new_moon_times)} ijtimak dalam rentang {start_date} hingga {end_date}")
        ...
        for ijtimak_time in new_moon_times:
            ...
            st.write(f"🔍 Memproses ijtimak: {ijtimak_date}")
            ...
            for day_offset in range(-1, 2):
                ...
                try:
                    # PERBAIKAN: Cari waktu sunset dengan metode yang lebih robust
                    ...
                    if sunset_t is None:
                        # Estimasi sunset jam 18:00 waktu lokal (UTC+7)
                        sunset_local = datetime(check_date.year, check_date.month, check_date.day, 18, 0, 0)
                        # Konversi ke UTC (UTC+7 -> -7 jam)
                        sunset_utc = sunset_local - timedelta(hours=7)
                        sunset_t = ts.utc(sunset_utc)
                    ...
                    # Illumination menggunakan formula yang benar
                    ...
                    st.write(f"  ✅ Data untuk {check_date}: Alt={alt_moon.degrees:.2f}°, Elong={elong:.2f}°")
                    
                except Exception as e:
                    st.write(f"  ❌ Error pada {check_date}: {str(e)}")
                    continue
...
if menu == "🔍 Deteksi Hilal":
    ...
    if mode == "Deteksi Gambar":
        ...
        with col2:
                ...
                if 'df_historical' in st.session_state and not st.session_state['df_historical'].empty:
                    ...
                    if selected_date:
                        ...
                        **Data untuk {selected_date}:**
                        ...
                        st.text_input("Altitude (°)", value=f"{altitude:.3f}", disabled=True, key="alt_readonly")
                        st.text_input("Elongasi (°)", value=f"{elongation:.3f}", disabled=True, key="elon_readonly")
                        st.text_input("Lebar (arcmin)", value=f"{width:.3f}", disabled=True, key="width_readonly")
            ...
            if st.button("🔬 Analisis Visibilitas", type="primary", key="analyze_btn"):
                q_value, visibility = calculate_hilal_visibility(altitude, elongation, width)

                if criteria == "MABIMS":
                    status = get_mabims_status(altitude, elongation)
                    st.info(f"Kriteria: MABIMS – Altitude={altitude:.3f}°, Elongasi={elongation:.3f}°")
        ...
        if image is not None and model is not None:
            if st.button("🚀 Deteksi Hilal (Gambar)", type="primary", key="detect_image_btn"):
        ...
        uploaded_video = st.file_uploader("Unggah video (.mp4, .avi, .mov)", type=["mp4", "avi", "mov"], key="video_uploader")

        if uploaded_video is not None and model is not None:
            process_button = st.button("🚀 Proses Video", type="primary", key="process_video_btn")
            if process_button:
...
# Menu 2: Data Historis
    ...
    with st.expander("⚙️ Pengaturan Rentang & Lokasi", expanded=True):
        ...
        generate = st.button("🔄 Generate Data Historis", type="primary", key="generate_hist_btn")
    ...
    if not df_historical.empty:
        ...
        col1, col2 = st.columns(2)
        with col1:
            ui_start = st.date_input("Filter Dari", value=df_historical['Tanggal'].min().date(), key="filter_start")
        with col2:
            ui_end = st.date_input("Filter Sampai", value=df_historical['Tanggal'].max().date(), key="filter_end")
...
# Menu 3: Informasi
    ...
    - Pilih sumber parameter: Input Manual atau Ambil dari Data Historis
    ...
    Klasifikasi (6 kategori sesuai standar Yallop):
    - **q ≥ +0.216**: Mudah terlihat dengan mata telanjang
    - **+0.216 > q ≥ -0.014**: Terlihat dalam kondisi atmosfer sempurna
    - **-0.014 > q ≥ -0.160**: Mungkin memerlukan alat bantu optik
    - **-0.160 > q ≥ -0.232**: Hanya terlihat dengan alat bantu optik
    - **-0.232 > q ≥ -0.293**: Tidak terlihat dengan teleskop konvensional
    - **q < -0.293**: Tidak terlihat (di bawah batas Danjon)
    
    #### MABIMS
    - **Altitude ≥ 3° DAN Elongasi ≥ 6.4°**: Kriteria terpenuhi
    ...
    - **Status Yallop**: Klasifikasi visibilitas berdasarkan q-value (6 kategori)
    ...
    **Versi 3.0** (Final Revision):
    - ✅ **Perbaikan Utama**: Filter ijtimak yang akurat - hanya 1 ijtimak per bulan
    - ✅ **Input Presisi Tinggi**: Ganti slider dengan input manual (3 desimal)
    - ✅ **Kalkulator Otomatis**: Ambil parameter langsung dari data historis
    - ✅ **Kriteria Yallop Lengkap**: 6 kategori sesuai standar ilmiah
    - ✅ **Debugging Real-time**: Informasi detail proses perhitungan
    - ✅ **Validasi Input**: Peringatan untuk nilai tidak wajar
    - ✅ **Sorting Data**: Data terurut berdasarkan tanggal
    - ✅ **Error Handling**: Fallback yang lebih robust
    
    ### 🐛 Perbaikan Bug Utama
    
    1. **Filter Ijtimak**: Sekarang hanya menampilkan ijtimak dalam rentang tanggal yang diminta
    2. **Timezone Confusion**: Perbaikan konversi waktu sunset
    3. **Input Presisi**: Input manual dengan presisi hingga 3 desimal
    4. **Klasifikasi Yallop**: 6 kategori sesuai standar ilmiah
    5. **Data Consistency**: Sorting dan filtering yang konsisten
    ...
    5. **Kalkulator Otomatis**: Generate data historis terlebih dahulu sebelum menggunakan fitur ini
    ...
    st.success("✨ **Aplikasi Final Siap Digunakan**: Semua perbaikan telah diterapkan untuk hasil yang akurat dan presisi tinggi!")
